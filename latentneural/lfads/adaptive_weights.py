...
class AdaptiveWeights(tf.keras.callbacks.Callback):
  def __init__(
    self,
    initial: List[tf.Variable]=[1., 1., 1.],
    min_weight: List[float]=[0., 0., 0.], 
    max_weight: List[float]=[1., 1., 1.], 
    update_steps: List[int]=[1, 1, 1], 
    update_rates: List[float]=[0., 0., 0.], 
    update_starts: List[int]=[0, 0, 0]):
    """
